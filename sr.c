...
// m68k-amigaos-gcc -O3 -noixemul -DUSECRC -Izlib-1.2.13/ -Lzlib-1.2.13/ -o sr sr.c -lz
        ...
        while (!abort)
        {
          ...
          if (CheckIO((struct IORequest *)SerialIO))
          {
            ...
            Printf("%ld/%ld bytes [%ld] %s     \r", sum, filesize,
                   sum * TICKS_PER_SECOND / ticks, (LONG)eta_to_human_short(rem, 0));
            if (sum >= filesize)
            {
